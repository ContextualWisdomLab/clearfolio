...
## 2026-08-28 - DocumentFragment DOM insertion
**Learning:** Appending elements one by one directly to the DOM within a loop (like `el.historyBody.appendChild(row)`) can cause performance issues because it triggers multiple layout reflows and repaints in the browser. Using a `DocumentFragment` batches these operations, improving rendering efficiency significantly for lists of elements.
**Action:** Next time I modify DOM lists, I will use `document.createDocumentFragment()` to accumulate elements inside the loop, and append the fragment to the DOM once outside the loop to optimize layout updates. Ensure tests correctly mock the DocumentFragment implementation.
