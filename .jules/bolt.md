...
## 2026-09-22 - DOM 일괄 업데이트를 통한 렌더링 성능 최적화
**Learning:** 루프 내에서 `appendChild`를 반복 호출하여 DOM에 직접 요소를 추가하면, 브라우저가 각 요소 추가마다 리플로우(reflow)와 리페인트(repaint)를 수행하게 되어 성능이 심각하게 저하될 수 있습니다.
**Action:** 여러 DOM 요소를 동적으로 생성하고 추가할 때는 `DocumentFragment`를 사용하여 메모리 상에서 먼저 조립한 뒤, 마지막에 단 한 번만 실제 DOM 트리에 일괄 추가하여 렌더링 오버헤드를 최소화해야 합니다.
