...
## 2026-09-06 - JWT/토큰 파싱 성능 최적화 (Fail-fast 서명 검증 및 불필요한 배열 할당 제거)
**Learning:** 정규식을 사용하는 `String.split()`과 배열 할당은 불필요한 객체 생성과 GC 압박을 초래합니다. 다만 서명 검증(HMAC) 전에 payload의 cardinality(필드 개수)를 검사하여 protected structural-admission 계약을 유지해야 합니다.
**Action:** 토큰의 서명 부분을 `lastIndexOf('.')`로 찾아 분리한 뒤, payload의 cardinality(필드 개수)를 먼저 확인하고 서명 검증을 수행하는 구조를 유지해야 합니다. 이후 간단한 구분자 기반 문자열 분리는 `split()` 대신 수동 루프와 `indexOf()`, `substring()`을 사용하여 불필요한 배열 및 스트림 오버헤드를 줄여야 합니다.
