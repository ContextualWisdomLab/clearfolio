...
## 2026-09-15 - 문자열 파싱 시 split() 및 Stream 할당 오버헤드 제거
**Learning:** 간단한 쉼표로 구분된 문자열을 파싱할 때 `String.split()`과 Java Stream API를 사용하면 불필요한 배열 할당과 함수형 래퍼 객체 생성으로 인해 가비지 컬렉션(GC) 압박과 오버헤드가 발생합니다.
**Action:** 단순한 구분자를 파싱할 때는 `String.indexOf()`와 `String.substring()`을 활용한 수동 루프를 사용하여 객체 할당을 최소화하고 성능을 높이도록 구현합니다.
