...
## 2026-09-22 - [직렬화 성능 개선]
**Learning:** Java에서 많은 문자열 필드를 연결할 때 `String.join()`을 사용하면 내부적으로 가변 인자 배열 생성 및 `String.valueOf()` 호출로 인해 불필요한 객체 할당이 발생하여 성능 병목이 될 수 있음을 확인했습니다.
**Action:** 여러 필드를 순차적으로 결합해야 하는 구조화된 데이터 직렬화 시에는 크기가 미리 지정된 `StringBuilder`와 체이닝된 `append()` 호출을 사용하여 중간 객체 할당을 방지해야 합니다.
