...
## 2026-08-30 - 토큰 파싱 시 Fail-fast 방식 및 문자열 분리 최적화 도입
**Learning:** `String.split()`과 Java Stream을 사용하여 간단하게 구분된 토큰을 파싱하면 불필요한 배열 할당 및 함수형 오버헤드가 발생하며, 유효하지 않거나 악의적으로 조작된 토큰에 대해 서명 검증보다 파싱을 먼저 수행할 경우 가비지 할당을 유발합니다.
**Action:** JWT나 토큰 파싱 시 서명(HMAC 등)을 가장 먼저 검증하는 fail-fast 구조를 채택하고, 단순 구분자를 사용할 때는 GC 부담을 줄이기 위해 `indexOf()`와 `substring()`을 활용한 수동 루프를 적용해야 합니다.
