...
## 2026-09-20 - Use StringBuilder instead of String.join for large concatenations
**Learning:** Concatenating many strings using `String.join` creates a varargs array and generates intermediate object allocations.
**Action:** When serializing ledger records or joining many fields, use a pre-sized `StringBuilder` with chained `append()` calls instead of `String.join` to avoid unnecessary memory allocation overhead.
