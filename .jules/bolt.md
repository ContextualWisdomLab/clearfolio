...
## 2026-09-16 - 단일 패스 문자열 분리 최적화 (O(N) 단일 스캔 및 split() 회피)
**Learning:** `String.split(",")`을 호출하면 단일 문자 구분자라도 배열 할당 및 정규식 평가 관련 비용이 발생하고, 이를 `Arrays.stream()`으로 감싸면 추가적인 객체 할당(스트림)이 일어납니다.
**Action:** 단순히 문자로 구분된 문자열을 파싱할 때는 `indexOf(',')`를 이용한 `while` 루프 단일 스캔을 통해 불필요한 배열과 스트림 생성을 방지하여 할당 오버헤드를 줄입니다.
