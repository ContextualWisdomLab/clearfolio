...
## 2026-07-31 - JWT Token Parsing
**Learning:** Extracting parts of a token via `String.split()` creates unnecessary intermediate string arrays which increases GC pressure.
**Action:** Use `String.lastIndexOf()` and `String.substring()` to efficiently extract payload and signature, then split only when needed.
