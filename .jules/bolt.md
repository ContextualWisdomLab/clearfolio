...
## 2026-08-09 - 토큰 파싱 성능 최적화: split, copyOf 대신 indexOf, substring 사용
**Learning:** 알려진 구분자(예: '.', '\t', ',')가 있는 JWT 페이로드나 토큰을 파싱할 때 `String.split()`과 `Arrays.copyOf()`를 사용하면, 서명이 검증되지 않거나 유효하지 않은 토큰 처리 시에도 불필요한 배열 할당 및 가비지 컬렉션(GC) 압박이 발생합니다.
**Action:** `indexOf()`, `lastIndexOf()`, `substring()`을 사용하여 서명 검증을 먼저 수행한 후, 필요한 경우에만 개별 필드를 추출하도록(fail-fast) 최적화하고, 추출할 항목 개수 제한을 넘을 경우 예외를 발생시키도록 하여 핫 패스에서의 메모리 할당 및 GC 오버헤드를 대폭 줄입니다.
## 2026-08-09 - 거절된 변경사항의 교훈: 벤치마크 없는 파서 재작성 및 다른 보안 PR과의 충돌
**Learning:** 측정된 벤치마크(할당/처리량) 증거나 포맷 동일성 테스트 없이 보안/지속성 경계에 걸쳐 파서를 재작성하는 성능 최적화는 보안 권한을 분산시키고 다른 PR(예: #276)과 충돌을 일으켜 거절됩니다.
**Action:** 향후 성능 최적화를 제안할 때는 측정된 핫스팟이 있는지 확인하고 벤치마크를 제공하며, 기존에 진행 중인 통합 파서/원장 컨트랙트나 보안 패치와 충돌하지 않도록 변경 범위를 좁히고 의미적 변화(semantic drift)가 없음을 증명하는 테스트를 포함해야 합니다.
