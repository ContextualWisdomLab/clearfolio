...
## 2026-09-03 - DOM 배치 삽입 최적화
**Learning:** 테이블 행과 같이 대량의 요소를 생성할 때 반복문 안에서 요소를 직접 DOM에 하나씩 추가하면 브라우저에서 다수의 리플로우(reflow)와 리페인트(repaint)가 발생하여 렌더링 성능이 크게 저하됩니다.
**Action:** 항상 `DocumentFragment`를 생성하여 반복문 안에서 자식 요소들을 fragment에 모은 뒤, 반복이 끝난 후 fragment를 DOM에 정확히 한 번만 삽입하도록 DOM 삽입을 일괄(batch) 처리해야 합니다.
