...
## 2026-08-29 - Fragment mocks need manual state management
**Learning:** When mimicking standard DOM behavior in custom Node.js DOM mocks (like DocumentFragment), you must explicitly implement native behaviors like clearing the `childNodes` array after an append. The actual DOM removes children from a fragment upon insertion, and failing to simulate this can cause tests to pass but diverge from real browser constraints.
**Action:** Always verify custom DOM mock behavior against standard web APIs, especially for node transfer operations, instead of just making tests pass.
