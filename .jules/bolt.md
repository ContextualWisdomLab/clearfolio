...
## 2026-09-21 - [DOM Batching with DocumentFragment]
**Learning:** 프론트엔드 JS 환경(바닐라 DOM API 사용 시)에서 렌더 함수 내부 루프에서 직접 `el.historyBody.appendChild(row)`를 반복 호출하면, 배열이 클 경우 여러 번의 브라우저 레이아웃 계산(thrashing)과 DOM 리플로우가 발생하여 성능이 저하됩니다. DOM 변이를 일괄 처리하기 위해 항상 `DocumentFragment`를 사용해야 합니다.
**Action:** `for` 루프 내에서의 반복적인 `.appendChild` 호출을 단일 `fragment.appendChild(row)` 패턴으로 변경하고, 마지막에 fragment를 DOM에 추가하도록 수정했습니다.
