...
## 2026-07-14 - InMemoryMultipartFile 방어적 복사 제거
**Learning:** `InMemoryMultipartFile`과 같이 요청당 생성되고 스프링 프레임워크에 의해 불변으로 취급되는 인메모리 바이트 배열 래퍼의 경우, 생성자 및 `getBytes()` 메서드에서 `Arrays.copyOf()`를 사용한 방어적 복사는 큰 파일 업로드 시 불필요한 메모리 할당 오버헤드 및 가비지 컬렉션(GC) 압박을 유발합니다.
**Action:** 인메모리 바이트 배열을 단순히 감싸는 용도의 클래스에서는 배열을 직접 할당 및 반환하도록 수정하여, 불필요한 방어적 복사로 인한 병목 현상을 방지하고 메모리 사용량과 성능을 최적화해야 합니다.
