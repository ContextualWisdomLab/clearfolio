...
## 2026-08-22 - 토큰 파싱 시 Fail-Fast 검증 및 메모리 할당 최적화
**Learning:** 서명 검증 전에 `String.split()`을 사용하여 JWT와 유사한 토큰의 모든 구성 요소를 파싱하면 불필요한 배열 및 객체 할당이 발생하며, 악의적으로 조작된 토큰을 처리할 때 가비지 컬렉션(GC) 압박을 심화시킬 수 있습니다.
**Action:** 전체 파싱은 서명 검증 이후로 지연시켜야 합니다. 먼저 `lastIndexOf('.')`와 `substring()`을 사용하여 서명과 페이로드를 추출하고 HMAC 서명을 검증한 후, `String.split()` 대신 엄격한 경계 검사(bounds checking)를 포함한 수동 `indexOf()` 루프를 사용하여 토큰을 분리함으로써 성능을 최적화하고 악성 입력에 대한 복원력을 높입니다.
