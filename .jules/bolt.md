...
## 2026-08-12 - Optimize ArtifactToken parsing in ArtifactLinkService
**Learning:** Replacing `String.split()` and `String.join()` with `lastIndexOf()` and `substring()` for parsing known token structures significantly reduces object allocation and GC pressure. Deferring the parsing of claims until after HMAC verification prevents allocating garbage strings for maliciously crafted or invalid tokens.
**Action:** Use `indexOf`/`lastIndexOf` and `substring` for token parsing and only allocate claims after signature validation in high-throughput endpoints.
