...
## 2026-07-25 - TOCTOU 및 중복 I/O 방지를 위한 단일 I/O 작업 수행 패턴
**Learning:** 파일 시스템 조작 시 `Files.exists()`를 먼저 호출하고 바로 이어서 파일을 읽는 패턴(`Files.readAllBytes()`, `Files.lines()` 등)은 I/O 비용(stat call)을 불필요하게 두 번 발생시킬 뿐만 아니라 TOCTOU(Time-Of-Check to Time-Of-Use) 취약점을 야기할 수 있습니다.
**Action:** `Files.exists()`로 존재 여부를 사전 확인하지 말고, 즉시 읽기 작업을 수행한 후 `java.nio.file.NoSuchFileException`을 잡아내어(catch) 캐시 미스나 파일이 없는 상황을 처리하도록 구현하여 성능과 안정성을 향상시켜야 합니다.
