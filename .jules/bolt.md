...
## 2026-09-14 - 프론트엔드 DOM 삽입 성능 최적화
**Learning:** 루프 내에서 DOM 요소에 직접 appendChild를 호출하면 매 삽입 시마다 브라우저의 reflow와 repaint가 발생하여 성능 저하의 원인이 됩니다.
**Action:** 여러 DOM 요소를 반복적으로 추가할 때는 DocumentFragment를 생성하여 여기에 모두 추가한 뒤, 마지막에 단 한 번만 문서에 삽입(배치 처리)하여 렌더링 성능을 최적화해야 합니다.
