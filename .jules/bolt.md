...
## 2026-07-28 - Reuse HexFormat instance to prevent repeated allocations
**Learning:** Calling `java.util.HexFormat.of()` repeatedly in methods like `sha256Hex` or `calculateSha256` creates unnecessary intermediate object allocations that negatively impact performance.
**Action:** Always use a single, reusable `private static final java.util.HexFormat HEX_FORMAT = java.util.HexFormat.of();` instance and its `formatHex` method for byte-to-hex string conversions.
