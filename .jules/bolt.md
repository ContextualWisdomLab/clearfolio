...
## 2026-09-07 - 문자열 파싱 시 split() 대신 indexOf() 사용
**Learning:** `String.split()`은 정규식을 사용하고, 중간 배열을 생성하기 때문에 성능에 오버헤드가 발생합니다. 특히 토큰 파싱이나 데이터 파일(Ledger) 처리와 같이 빈번하게 호출되는 로직에서는 `indexOf()`와 `substring()`을 조합하여 단일 패스로 처리하고, 불필요한 배열 및 객체 할당을 줄이는 것이 좋습니다.
**Action:** 단순한 구분자를 사용해 문자열을 파싱할 때는 `split()` 대신 수동 루프와 `indexOf()`를 사용하여 메모리 할당(GC 압박)을 줄이고 파싱 성능을 향상시킵니다.
