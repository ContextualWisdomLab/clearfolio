...
## 2024-08-01 - JWT 토큰 파싱 성능 최적화
**Learning:** JWT 토큰이나 dot(.)으로 구분된 문자열을 파싱할 때 `String.split()`을 사용한 후 서명 검증을 위해 `Arrays.copyOf()`와 `String.join()`으로 문자열을 재조합하는 것은 불필요한 배열 생성과 문자열 조작을 발생시켜 GC(Garbage Collection) 부하를 가중시킨다.
**Action:** 구분자의 위치가 명확하거나 마지막 요소만 분리해야 하는 경우, `indexOf` 또는 `lastIndexOf`와 `substring`을 사용하여 직접 분리함으로써 배열 생성 및 객체 할당을 최소화한다.
