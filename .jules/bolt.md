...
## 2026-09-11 - Use single scan indexOf loop for simple delimiter string processing instead of split and stream
**Learning:** Using `String.split()` coupled with Java Streams (`Arrays.stream(...).map(...).filter(...)`) for parsing simple comma-separated string payloads creates unnecessary internal array allocations and functional pipeline overhead.
**Action:** When parsing simple delimiter separated tokens and manipulating them, use a manual `while` loop with `indexOf()` and `substring()` to extract the tokens and apply logic (like `sanitize`) conditionally. This avoids allocating full arrays and stream objects, significantly reducing GC pressure on hot paths like authentication claim extraction.
