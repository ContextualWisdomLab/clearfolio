...
## 2026-07-30 - JWT/토큰 파싱 최적화 (불필요한 문자열 할당 방지)
**Learning:** JWT 또는 토큰 페이로드를 재구성하기 위해 `String.join`과 `Arrays.copyOf`를 결합하여 사용하거나, `String.split()`을 정규식 기반으로 호출하는 것은 불필요한 문자열 배열 및 객체 할당을 발생시켜 GC(가비지 컬렉션) 압박을 줍니다. 점(.) 등 구분자를 기반으로 한 고정된 토큰 구조를 다룰 때에는 `indexOf()`, `lastIndexOf()`, `substring()`을 조합하여 직접 페이로드와 서명을 분리하고 파싱하는 것이 메모리 효율성과 속도 측면에서 월등히 유리합니다.
**Action:** 전체 토큰 문자열에서 페이로드와 서명을 검증하고 파싱할 때, 불필요한 `split()`, 배열 복사, `join()`을 지양하고 `lastIndexOf`와 루프 내 `indexOf`를 활용하여 단일 패스(혹은 최소한의 패스)로 필요한 문자열만을 추출하도록 로직을 최적화하여 핫 패스(hot path)의 성능을 향상시켜야 합니다.
