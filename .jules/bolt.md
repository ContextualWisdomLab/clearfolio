...
**Learning:** `ByteArrayInputStream`은 제공된 바이트 배열을 내부적으로 변경하지 않으므로, 생성자에서 이미 복사해 보관한 내부 배열을 다시 `Arrays.copyOf()`로 복사하면 대용량 업로드에서 불필요한 메모리 할당과 GC 부하가 생깁니다.
**Action:** 내부 바이트 배열이 클래스 외부로 직접 노출되지 않는 경우, `getInputStream()`은 원본 내부 배열을 감싼 `ByteArrayInputStream`을 반환해 반복 호출 비용을 줄입니다.
