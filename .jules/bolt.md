...
## 2026-07-14 - Use StringBuilder for structured string concatenation to reduce allocations
**Learning:** Using `String.join()` with many concatenated string fields creates unnecessary varargs array creation and intermediate object allocations.
**Action:** When serializing structured ledger records in Java with many concatenated string fields, use a pre-sized `StringBuilder` with chained `append()` calls rather than `String.join()` to avoid unnecessary object allocations and improve performance.
