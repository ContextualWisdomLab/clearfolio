...
## 2026-09-26 - Optimized DOM rendering in demo.js
**Learning:** Sequential DOM insertions in a loop trigger multiple reflows/repaints, which is a common frontend performance bottleneck, especially in tables like the history view. Mocking `DocumentFragment` for frontend tests requires updating the test environment's `globalThis.document` mock to properly simulate `DocumentFragment` insertion behavior and ensure 100% test coverage.
**Action:** Use `DocumentFragment` to batch DOM insertions when rendering lists/tables to improve rendering performance. Ensure custom test mocks (like `mock-dom.mjs`) are properly updated to support `MockDocumentFragment` and its integration with `appendChild` and `append` to avoid test crashes.
