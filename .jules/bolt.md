...
## 2026-08-24 - JWT/Token 파싱 시 정규식 기반 split 및 join 회피를 통한 성능 최적화
**Learning:** `String.split()`과 `String.join()`을 사용하여 단순한 구분자(delimiter) 기반의 토큰을 파싱하는 것은 불필요한 배열 생성, 정규식 컴파일, 그리고 가비지 컬렉션 오버헤드를 유발하여 성능 저하의 원인이 됩니다.
**Action:** 간단한 구분자로 이루어진 토큰(예: JWT, ArtifactToken)을 파싱할 때는 수동으로 `indexOf()`와 `substring()`을 사용하는 루프를 구현하여 메모리 할당을 최소화하고 처리 속도를 개선해야 합니다. 특히 서명(Signature) 검증을 먼저 수행하는 fail-fast 구조를 적용하면 잘못된 토큰 처리 시 발생하는 불필요한 객체 생성을 원천 차단할 수 있습니다.
