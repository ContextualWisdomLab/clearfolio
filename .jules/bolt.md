...
## 2024-05-24 - [String.join() over StringBuilder]
**Learning:** String.join() creates an intermediate varargs array and performs string concatenation in a way that is less efficient than manually using a pre-sized StringBuilder with chained append() calls, especially for many fields.
**Action:** When serializing structured ledger records with many concatenated string fields, use a pre-sized StringBuilder with chained append() calls rather than String.join() to avoid unnecessary varargs array creation and intermediate object allocations.
