...
## 2026-07-14 - DOM Fragment 성능 최적화
**Learning:** 루프 내에서 DOM 요소에 `appendChild`를 반복적으로 호출하면 불필요한 리플로우(reflow)와 리페인트(repaint)가 발생하여 렌더링 성능이 저하됩니다.
**Action:** 여러 DOM 요소를 삽입할 때는 항상 `DocumentFragment`를 사용하여 메모리 내에서 요소를 배치한 후 한 번에 DOM에 삽입하여 성능을 최적화하고 렌더링 블로킹을 방지해야 합니다.
## 2026-08-19 - 최적화 근거 부재 및 권한(Domain) 경계 위반으로 인한 PR 기각
**Learning:** 데모 전용 코드(`demo.js`)에서의 DOM 최적화는 향후 제거되거나 고립될 예정인 영역(issue #317)이며, 실제 프로덕트 렌더링 성능에 유의미한 영향을 주지 않습니다. 구체적인 브라우저 벤치마크, 트레이스, 또는 전/후 성능 증거 없이 렌더링 향상 수치를 주장하는 것은 수용되지 않으며, 성능에 민감한 상태 관리는 디자인 시스템(#476)이나 실제 프로덕트 영역에서 이뤄져야 합니다.
**Action:** 앞으로 성능 최적화를 수행할 때는 반드시 벤치마크나 성능 측정 증거를 제시해야 하며, 시연용 데모 코드가 아닌 실제 성능이 중요한 프로덕트 워크스페이스나 디자인 시스템 코어 등 적절한 대상(product authority)을 선택하여 마이크로 최적화를 진행해야 합니다.
