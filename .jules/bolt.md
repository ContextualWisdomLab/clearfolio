...
## 2026-07-29 - TOCTOU 최적화 및 I/O 성능 향상
**Learning:** 파일을 읽기 전에 `Files.exists()`로 존재 여부를 확인하는 것은 Time-Of-Check to Time-Of-Use (TOCTOU) 경쟁 조건을 발생시킬 수 있으며, 디스크 I/O를 불필요하게 두 번 수행하게 되어 성능 저하를 일으킵니다.
**Action:** 앞으로 파일을 읽을 때는 존재 여부를 먼저 확인하지 않고, 즉시 읽기를 시도(`Files.readAllBytes()` 등)한 후 `java.nio.file.NoSuchFileException`을 catch하여 파일이 없는 경우를 처리하도록 코드를 작성하겠습니다.
