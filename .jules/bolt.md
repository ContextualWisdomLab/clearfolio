...
**Learning:** 정규식을 사용하는 `String.split()`과 배열 할당은 불필요한 객체 생성과 GC 압박을 초래합니다. 특히 서명 검증(HMAC) 전에 각 필드를 분리하여 배열을 생성하고 이를 다시 합치는 작업은 유효하지 않거나 악의적으로 조작된 토큰 요청 시 가비지를 대량으로 양산합니다.
**Action:** 토큰의 서명 부분을 `lastIndexOf('.')`로 찾아 분리한 뒤 즉시 서명 검증을 먼저 수행하는 fail-fast 접근 방식을 적용해야 합니다. 또한, 간단한 구분자 기반 문자열 분리는 `split()` 대신 수동 루프와 `indexOf()`, `substring()`을 사용하여 불필요한 배열 및 스트림 오버헤드를 줄여야 합니다.
