...
## 2026-08-06 - 문자열 파싱 및 배열 할당 성능 최적화 (split 제거)
**Learning:** 정해진 구분자로 문자열을 나눌 때 `String.split()`과 `Arrays.copyOf()` 등을 사용하면 매번 불필요한 배열 생성 및 복사가 일어나 가비지 컬렉터(GC) 압박이 증가하며 성능이 저하됩니다.
**Action:** 알려진 구분자(예: 탭, 쉼표, 점 등)를 사용하는 토큰 및 로그 파일 파싱 시, `split()` 대신 `indexOf()`와 `substring()`을 이용한 수동 파싱을 사용하고, 엄격한 배열 경계 검사(bounds checking)를 통해 데이터 손실을 방지하고 불필요한 할당을 제거해야 합니다.
