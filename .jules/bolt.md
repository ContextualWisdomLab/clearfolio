...
## 2026-07-14 - 토큰 파싱 시 지연 할당 및 조기 검증 최적화
**Learning:** JWT나 유사한 커스텀 토큰 파싱 시 `String.split()`을 먼저 호출하여 모든 필드 문자열 배열을 할당하는 것은 악의적이거나 유효하지 않은 토큰 공격 시 가비지 컬렉션(GC) 압박과 불필요한 메모리 할당 오버헤드를 발생시킵니다.
**Action:** 시그니처 검증 등 비용이 적고 중요한 보안 검증을 수행하기 전까지는 `indexOf()`와 `substring()`을 조합하여 개별 필드의 할당을 지연시키는(Fail-fast) 최적화를 통해 메모리와 성능을 방어해야 합니다.
