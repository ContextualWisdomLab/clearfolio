...
## 2026-08-25 - DOM 삽입 성능 개선을 위한 DocumentFragment 활용
**Learning:** DOM에 엘리먼트를 순차적으로 여러 번 append하면 브라우저의 리플로우와 리페인트가 반복되어 렌더링 성능이 저하됩니다.
**Action:** 여러 엘리먼트를 DOM에 삽입해야 할 때는 `DocumentFragment`를 사용하여 메모리 상에서 배치를 구성한 후 한 번에 DOM 트리에 추가해야 합니다.
