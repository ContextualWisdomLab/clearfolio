...
## 2026-08-14 - Optimize Artifact Token Parsing
**Learning:** Using `String.split()` for simple delimited tokens incurs unnecessary array allocation and regex compilation overhead.
**Action:** Replaced `String.split` with a manual `indexOf()` loop in `ArtifactLinkService.java`, which improves performance and reduces GC pressure while preserving accurate token parsing logic.
