...
## 2026-09-18 - [Optimized DOM Insertions]
**Learning:** In a vanilla JS environment, appending elements one by one directly to the DOM causes multiple reflows, which degrades frontend performance.
**Action:** Batch DOM manipulations into a `DocumentFragment` first and then append it to the main DOM tree. Note that this requires updating test mocks (e.g. `mock-dom.mjs`) to properly handle `DocumentFragment` correctly.
