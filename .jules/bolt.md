...
## 2026-08-11 - JWT/토큰 파싱 성능 최적화
**Learning:** HMAC 서명이 검증된 *이후*에 개별 토큰 필드의 추출 및 배열 할당을 지연해야 합니다. 이 '빠른 실패(fail-fast)' 접근법은 악의적으로 조작되거나 유효하지 않은 토큰을 처리할 때 파싱된 클레임에 대한 가비지 할당을 방지합니다.
**Action:** 부분 문자열 연산(예: 페이로드와 서명을 분리하는 점을 찾기 위해 lastIndexOf 사용)을 사용하여 예상되는 서명을 추출하고 검증을 수행합니다. 서명이 일치하는 경우에만 문자열 분리(예: payload.split("\\.", -1))를 수행합니다.
