...
## 2026-08-06 - JWT/Token 파싱 시 서명 검증 전 메모리 할당 지연 (Fail-fast)
**Learning:** 서명을 검증하기 전에 `String.split()`을 호출하여 토큰의 모든 필드를 추출하고 배열을 생성하면, 유효하지 않거나 악의적으로 조작된 토큰을 처리할 때 불필요한 가비지(GC)가 대량으로 발생하여 성능에 악영향을 미칩니다.
**Action:** 토큰을 파싱할 때 `lastIndexOf()`와 `substring()`을 사용하여 페이로드와 서명을 먼저 분리하고 서명 검증을 가장 먼저 수행합니다(Fail-fast). 개별 필드 추출 및 배열 할당은 서명 검증이 성공한 이후로 지연시켜야 합니다.
