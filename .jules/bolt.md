...
## 2026-09-02 - JWT/토큰 파싱 성능 최적화: 정규식 기반 split 제거
**Learning:** `String.split("\\.")`을 사용하여 토큰을 파싱하면 매 호출 시 내부적으로 불필요한 배열과 정규식 객체가 생성되어 가비지 컬렉션(GC) 압박이 증가하고 메모리 할당 오버헤드가 발생합니다. 특히 토큰이 유효하지 않은 경우에도 불필요한 배열을 미리 생성하게 됩니다.
**Action:** 단순 문자열 분리(예: JWT, JWS, HMAC 토큰 파싱)에는 `split()` 대신 `indexOf()`와 `substring()`을 조합하여 명시적 루프 안에서 처리함으로써 메모리 할당과 GC 오버헤드를 최소화하고 성능을 개선해야 합니다.
