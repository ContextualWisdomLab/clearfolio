...
## 2024-05-15 - Defer JWT/token field allocation and use indexOf/substring instead of split
**Learning:** Deferring field extraction until after HMAC verification prevents GC pressure for maliciously crafted or invalid tokens. Using `indexOf`, `lastIndexOf`, and `substring` to extract payload and signature directly reduces string and array allocations compared to `String.split()` and `Arrays.copyOf()`. Ensure bounds checking when iterating.
**Action:** Always prefer manual `indexOf`/`substring` parsing over `String.split()` for hot-path delimited token validation, and perform cryptographic checks before allocating memory for individual fields to fail fast on garbage input.
