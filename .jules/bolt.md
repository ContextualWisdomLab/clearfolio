...
## 2026-08-02 - Token Parsing 문자열 분할(Split) 최적화
**Learning:** JWT나 토큰을 파싱할 때 `String.split()`을 사용하고 `Arrays.copyOf()`와 `String.join()`으로 페이로드를 재구성하는 것은 불필요한 문자열 배열 할당과 결합 오버헤드를 유발하여 GC 압박을 증가시킨다.
**Action:** 특정 구분자로 나누어진 토큰의 페이로드와 서명을 분리할 때는 `String.lastIndexOf()`와 `String.substring()`을 활용하여 직접 추출함으로써 성능을 최적화하고 메모리 할당을 줄인다.
