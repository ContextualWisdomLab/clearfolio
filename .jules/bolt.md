...
## 2026-09-12 - String.split() 및 Stream 객체 할당 오버헤드 제거
**Learning:** `String.split()`과 Java Stream API를 연결하여 사용하는 구조는 편리하지만, 배열 할당 및 람다 래퍼 등 불필요한 객체를 다수 생성합니다. 특히 헤더 파싱과 같이 매 요청마다 호출되는 핫 패스(hot path)에서는 GC(가비지 컬렉터) 압박을 크게 높여 성능 저하의 원인이 됩니다.
**Action:** 핫 패스에서 단순 구분자로 문자열을 분리해야 할 경우, `String.split()` 대신 수동 `indexOf()` 및 `substring()` 루프를 사용하여 배열 할당과 함수형 오버헤드를 완전히 제거해야 합니다.
