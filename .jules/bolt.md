...
## 2026-08-13 - [Optimize delimited token parsing]
**Learning:** Using `String.split()` and Java Streams for parsing simple delimited tokens in hot paths creates unnecessary array allocations and functional overhead, leading to increased GC pressure.
**Action:** When parsing tokens or JWT payloads with a known delimiter (e.g., dots or commas), avoid `String.split()`, `Arrays.copyOf()`, and `String.join()`. Instead, use manual loops with `indexOf()`, `lastIndexOf()`, and `substring()` to extract segments directly.
