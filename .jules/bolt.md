...
## 2026-07-14 - 토큰 파싱 시 문자열 배열 및 복사 오버헤드 제거
**Learning:** `String.split()`, `Arrays.copyOf()`, `String.join()`을 사용하여 토큰(예: JWT, 자체 포맷)을 파싱하는 방식은 정규식 컴파일, 불필요한 객체 할당, 배열 복사를 유발하여 GC 압박을 심화시킵니다.
**Action:** 미리 정의된 구분자(예: `.`)를 기준으로 파싱할 때는 `lastIndexOf()`, `indexOf()`, `substring()`을 사용하여 문자열 및 배열 할당을 최소화함으로써 핫 패스의 성능을 개선해야 합니다.
