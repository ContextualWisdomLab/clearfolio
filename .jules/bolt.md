...
## 2026-07-14 - 프론트엔드 DOM 렌더링 최적화 (DocumentFragment 사용)
**Learning:** DOM에 반복적으로 엘리먼트를 추가할 때마다 리플로우(reflow)와 리페인트(repaint)가 발생하여 성능이 저하됩니다. 특히 긴 리스트나 표의 행을 추가할 때 그렇습니다.
**Action:** 여러 엘리먼트를 DOM에 추가해야 할 때는 `document.createDocumentFragment()`를 생성하여 모든 요소를 fragment에 추가한 뒤, 마지막에 fragment를 실제 DOM에 한 번만 추가하도록 최적화하여 렌더링 성능을 향상시켜야 합니다.
