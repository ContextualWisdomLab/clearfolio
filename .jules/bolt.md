...
## 2026-08-23 - DocumentFragment를 이용한 DOM 업데이트 성능 최적화
**Learning:** 프론트엔드 JavaScript에서 배열을 순회하며 요소를 문서에 직접 추가하면(`appendChild` 루프) 매 요소마다 브라우저의 reflow/repaint가 발생하여 성능을 저하시킵니다.
**Action:** DOM 요소들을 반복적으로 삽입할 때는 `DocumentFragment`를 생성하여 반복문 내에서 모든 요소를 프래그먼트에 추가한 후, 마지막에 프래그먼트를 문서에 한 번만 삽입(append)함으로써 렌더링 성능을 최적화해야 합니다. 또한, 이를 위해 모의 테스트 DOM 구현체(`mock-dom.mjs`)에 프래그먼트 지원을 추가하여 테스트 커버리지 및 호환성을 유지해야 합니다.
