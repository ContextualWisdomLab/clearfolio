...
## 2024-07-28 - I/O 성능 최적화: TOCTOU 방지
**Learning:** `Files.exists()`를 먼저 호출하고 파일을 읽는 것은 디스크 접근을 두 번 하게 만들 뿐만 아니라 TOCTOU (Time-Of-Check to Time-Of-Use) 경쟁 조건을 발생시킬 수 있다.
**Action:** 파일이 존재하지 않는 것을 처리해야 할 경우, 파일을 직접 읽은 뒤 발생하는 `java.nio.file.NoSuchFileException`을 잡아서 처리하도록 하여 디스크 I/O를 줄이고 성능을 향상시킨다.
