...
## 2026-08-22 - DOM 요소 반복 삽입 시 DocumentFragment를 통한 렌더링 성능 최적화
**Learning:** 루프 내에서 DOM 요소에 노드를 순차적으로 직접 삽입하면, 각 삽입마다 브라우저가 reflow 및 repaint를 발생시켜 렌더링 성능이 크게 저하될 수 있습니다.
**Action:** 여러 DOM 요소를 동적으로 생성하여 삽입할 때는 `DocumentFragment`를 생성하여 루프 내에서 이 메모리 상의 파편에 노드들을 추가하고, 루프가 끝난 뒤 단 한 번만 실제 DOM에 파편을 삽입하도록 하여 불필요한 reflow와 repaint를 방지해야 합니다.
