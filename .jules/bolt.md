...
## 2026-08-23 - JWT 파싱 및 서명 검증 로직 성능 최적화
**Learning:** JWT 토큰 파싱 시 `String.split("\\.", -1)`을 사용하여 모든 클레임을 배열로 먼저 추출하는 것은 잘못된 서명이나 유효하지 않은 토큰에 대해서도 불필요한 객체 할당(가비지 컬렉션 부담)과 연산을 유발합니다.
**Action:** 서명 검증을 가장 먼저 수행하도록 로직을 재배치(Fail-fast)하여, 서명이 유효한 경우에만 나머지 클레임을 `indexOf`와 `substring`을 활용한 수동 루프로 파싱하여 메모리 할당 오버헤드를 최소화하고 성능을 최적화했습니다.
