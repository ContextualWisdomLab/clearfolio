...
## 2026-07-29 - Reusing HexFormat constants avoids memory allocations
**Learning:** HexFormat instances like `HexFormat.of()` are immutable and thread-safe. Calling `HexFormat.of()` inline on hot paths, like cryptographic digest calculation or string parsing, introduces unnecessary object allocation.
**Action:** Always replace inline `HexFormat.of()` invocations with a single reused `private static final HexFormat HEX_FORMAT = HexFormat.of();` constant.
