...
## 2026-09-13 - TenantContext 파싱 성능 최적화
**Learning:** 단순한 구분자를 사용한 문자열 분리 시 `String.split()`과 Java Stream을 사용하면 불필요한 배열 및 객체 할당이 발생하여 GC 부담이 커집니다.
**Action:** 성능에 민감한 헤더 파싱 로직 등에서는 `String.split()` 대신 `indexOf()`와 `substring()`을 활용한 수동 루프를 사용하여 오버헤드를 줄입니다.
