...
## 2026-08-29 - Token 파싱 최적화 및 지연 할당
**Learning:** JWT 등의 토큰 파싱 시 String.split()은 불필요한 배열 및 객체 할당을 발생시키며, 유효하지 않은 서명의 토큰에 대해서도 비용이 듭니다.
**Action:** 토큰 처리 시 indexOf와 substring을 사용하여 수동으로 파싱하고, 개별 클레임 할당 전 서명 검증을 먼저 수행(Fail-fast)하여 GC 압박을 줄입니다.
