...
## 2023-10-27 - [Avoid String.split and join for token parsing]
**Learning:** Using `String.split` and `String.join` for parsing delimited tokens generates high garbage collection overhead.
**Action:** Use `indexOf`, `lastIndexOf`, and `substring` to parse delimited strings on hot paths like token parsing to reduce GC pressure and allocations.
