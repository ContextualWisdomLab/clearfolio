...
## 2024-05-18 - 비동기 뷰어 로딩 상태 복원
**Learning:** 비동기 폴링 중 버튼 로딩 상태를 관리할 때 텍스트를 단순 변경하면 DOM 구조가 파괴되어 문제가 생길 수 있습니다.
**Action:** 뷰어 초기화 및 폴링에서 `setBusyState`를 사용하여 안전하게 로딩 상태를 관리하고 `finally`에 준하는 시점에서 복원합니다.
