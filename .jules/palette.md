...
## 2026-07-18 - Context-specific ARIA labels for table actions
**Learning:** Repetitive table action buttons or links (e.g., 'Details', 'Open') without context-specific labels are highly confusing for screen reader users, who hear the same generic text repeated for every row.
**Action:** Always provide dynamically generated, context-specific `aria-label` attributes that include row-specific identifiers (like the filename) for repetitive table actions.
