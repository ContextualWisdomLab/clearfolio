...
## 2026-08-27 - Grid Layouts and Inline Elements
**Learning:** When adding inline visual elements (like required indicators) inside a parent container styled with `display: grid` (such as `.field-label`), the grid layout can force the text and the new element onto separate rows, breaking the visual flow.
**Action:** Always wrap the text and the new inline element in an inner `<span>` to maintain them on the same line within the grid cell.
