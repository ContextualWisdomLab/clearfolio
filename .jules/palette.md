...
## 2026-08-01 - Improve accessibility for table actions
**Learning:** For repetitive table action buttons or links (e.g., 'Details', 'Open'), screen reader users need context-specific `aria-label` attributes that include row-specific identifiers (like the filename).
**Action:** Added optional `ariaLabel` parameters to UI component constructors (`createActionButton`, `createLink`) and passed dynamically generated, context-specific labels from the table rows.
