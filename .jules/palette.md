...
## 2026-08-10 - Generic Action Buttons Context
**Learning:** UX & Accessibility Convention: For dynamically generated lists or data tables with repeated generic action buttons (e.g., 'Details', 'Status JSON'), screen reader users lack context on what item the button acts upon if only the text is read.
**Action:** Always add contextual information (such as the specific entity's filename or title) into an `aria-label` attribute to provide critical context for screen reader users.
