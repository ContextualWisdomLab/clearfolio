...
## 2026-07-30 - Context-specific ARIA labels for history table actions
**Learning:** Repetitive table action buttons or links (e.g., 'Details', 'Open') lack context for screen reader users when multiple rows exist.
**Action:** Always provide dynamically generated, context-specific aria-label attributes that include row-specific identifiers (like the filename) to aid screen reader users. When adding optional parameters to existing utility functions, always append the new parameters to the end of the method signature.
