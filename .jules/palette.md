...
## 2026-09-09 - Accessible Required Indicators
**Learning:** When adding visual required indicators (e.g., `*`) to form fields that already use the HTML5 `required` attribute, they must include `aria-hidden="true"` to prevent redundant and confusing screen reader announcements.
**Action:** Always add `aria-hidden="true"` to visual required indicators when the associated input already has a `required` attribute.
