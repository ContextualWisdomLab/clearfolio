...
## 2024-08-09 - Contextual ARIA labels for table actions
**Learning:** Screen reader users lose context in repeating data tables when generic action buttons (like 'Details' or 'Status JSON') lack contextual identifiers.
**Action:** For dynamically generated lists or data tables with repeated generic action buttons, always add contextual information (such as the specific entity's filename or title) into an 'aria-label' attribute to provide critical context for screen reader users.
