...
## 2026-08-08 - Contextual ARIA labels for table actions
**Learning:** For dynamically generated lists or data tables with repeated generic action buttons (e.g., 'Details', 'Status JSON'), providing contextual information (such as the specific entity's filename or title) into an 'aria-label' attribute is critical for screen reader users to distinguish between the buttons.
**Action:** Always add contextual information into an 'aria-label' attribute for generic action buttons in repeated data rows.
