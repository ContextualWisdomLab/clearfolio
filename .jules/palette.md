...
## 2026-08-06 - Contextual ARIA labels for dynamic lists
**Learning:** Screen reader users lose context when navigating tables with repeated generic action buttons (like "Details" or "Status JSON") in every row.
**Action:** Always interpolate contextual identifiers (e.g., filename or entity ID) into `aria-label` attributes for list action buttons to restore context.
