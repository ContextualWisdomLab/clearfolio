...
## 2026-09-09 - Async Button Loading States in Viewer
**Learning:** Adding explicit loading and disabled states to asynchronous action buttons like "Refresh" in the viewer UI provides immediate feedback. To prevent losing nested DOM states or ARIA attributes, we should use the existing `setBusyState` utility.
**Action:** Always use `setBusyState(button, loadingText)` from `dom-utils.js` when modifying button states during asynchronous operations in the clearfolio viewer.
