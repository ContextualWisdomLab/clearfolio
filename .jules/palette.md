...
## 2026-09-15 - Required Indicator Grid Layout
**Learning:** When adding inline visual elements (like a required indicator) inside a parent container styled with `display: grid` (such as `.field-label`), they must be wrapped together with the text in an inner `<span>` to prevent the grid layout from forcing them onto separate rows.
**Action:** Always wrap text and inline indicators in a parent `<span>` when modifying grid-styled labels in this app.
