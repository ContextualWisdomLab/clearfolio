...
## 2024-08-08 - Missing Authentication on Admin Endpoints
**Vulnerability:** The AdminController endpoints lacked authentication and tenant scope validation, allowing unauthenticated access and potential cross-tenant data leakage.
**Learning:** Even endpoints prefixed with "admin" need explicit security barriers. Spring WebFlux does not implicitly secure these unless configured, and tenant scope must be manually enforced to prevent IDOR.
**Prevention:** Always use `TenantAccessService.require()` to enforce authentication and permissions on every endpoint, and `tenantAccessService.requireSameTenant()` or `belongsToTenant()` to explicitly verify resource ownership.
