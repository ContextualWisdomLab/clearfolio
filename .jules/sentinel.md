...
## 2026-08-24 - Unauthenticated and Unauthorized Admin Endpoints
**Vulnerability:** The endpoints in `AdminController.java` (`/api/v1/admin/convert/jobs`, etc.) were completely unauthenticated and unauthorized, leaking conversion jobs across all tenants and allowing unauthenticated users to delete or retry jobs.
**Learning:** Endpoints meant for "admin" use were not integrated with the `TenantAccessService`, leading to a complete bypass of the application's authorization boundary. Furthermore, the `getAllJobs` endpoint returned all jobs across the platform rather than filtering by the authenticated tenant's ID.
**Prevention:** Ensure that all new controllers, especially those performing sensitive administrative actions, inject and enforce `TenantAccessService` checks for both endpoint permissions (e.g., `TenantPermissions.JOB_READ`) and data-level tenant isolation (`belongsToTenant`).
