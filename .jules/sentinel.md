...
## 2026-07-02 - 관리자 API 인증 및 테넌트 격리 누락
**Vulnerability:** AdminController의 엔드포인트(/api/v1/admin/convert/jobs 등)에 인증, 권한 부여(TenantAccessService) 및 테넌트 격리 검증이 누락되어 누구나 모든 테넌트의 작업에 접근하고 수정할 수 있었습니다. 초기 수정에서는 저엔트로피 운영자 식별자를 unkeyed SHA-256으로 가명화해 사전 대입으로 복원할 수 있는 추가 개인정보 보호 결함도 남았습니다.
**Learning:** 관리자 API도 인증·권한·테넌트 소유권을 fail-closed로 검증해야 하며, 감사 상관 식별자는 plaintext나 unkeyed digest가 아니라 별도 감사 키와 목적별 domain separation을 적용한 keyed HMAC으로 생성해야 합니다.
**Prevention:** TenantAccessService로 요청 권한과 리소스 소유권을 검증하고, 재시도 운영자 식별자는 `RetryOperatorIdentityPort`를 통해 전용 audit pseudonym key/version과 retry 전용 domain으로 가명화합니다. 전용 키가 없으면 raw identifier나 unkeyed hash로 fallback하지 않습니다.
