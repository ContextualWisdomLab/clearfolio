...
## 2026-08-21 - Denial of Service via Long Token in parseAndVerify
**Vulnerability:** The \`parseAndVerify\` method in \`ArtifactLinkService.java\` lacks an upper bound on token length before performing expensive HMAC-SHA256 operations. An attacker can craft a massive token payload (e.g., megabytes) and force the server to perform linear-time hashing on each request, causing severe CPU exhaustion and Denial of Service.
**Learning:** Security scanners like Strix (and manual testers) identify missing input length bounds on cryptographic operations as critical resource exhaustion (CWE-400) vulnerabilities. We must validate input length limits *before* any resource-intensive operation (like hashing).
**Prevention:** Add explicit length bounds checking (e.g., \`token.length() > 4096\`) at the very beginning of the token verification method.
