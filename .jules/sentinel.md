...
**Learning:** 애플리케이션의 다른 컨트롤러에서는 `TenantAccessService`를 사용하여 수동으로 권한을 검사하고 있으나, AdminController에는 이 검사 로직이 주입되지 않아, 요청 헤더만 알면 누구나 관리자 권한을 행사할 수 있는 보안 취약점이 있었습니다.
