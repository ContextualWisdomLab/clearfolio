...
## 2026-08-15 - Remove Environment Variable Fallbacks for Secrets
**Vulnerability:** Runtime secrets were injected from environment variables using placeholders with empty fallbacks, violating the org rule to use a KV registry.
**Learning:** This bypasses strict mounting rules and can silently default to empty strings, which could lead to weak or missing security signatures.
**Prevention:** Remove environment variable fallback syntax from YAML configs and Java `@Value` annotations, enforcing the Spring Boot configtree mounting mechanism.
