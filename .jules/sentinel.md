...
## 2026-07-14 - Use bounds checking when parsing tokens with manual string traversal
**Vulnerability:** Replacing `String.split()` with manual `indexOf` parsing for delimited tokens can lead to logic flaws if bounds checking is not properly enforced, potentially ignoring trailing garbage fields or bypassing structural validation.
**Learning:** In Java, when manually parsing delimited strings using loops and `indexOf()`, it is critical to verify that the extracted element count precisely matches expectations. Failing to check if extra fields exist after the expected number of tokens has been parsed can leave the application vulnerable to maliciously crafted tokens.
**Prevention:** Always verify that no further delimiters exist beyond the final expected token (e.g., checking if `dotIndex != -1` when `i == TOKEN_FIELD_COUNT - 1`) to ensure strict structural conformity.
