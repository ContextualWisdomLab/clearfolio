...
## 2026-09-22 - [CRITICAL] Fix missing authentication on admin endpoints
**Vulnerability:** The `/api/v1/admin/convert/jobs` endpoints in `AdminController.java` lacked any tenant claim or permission checks, meaning anyone could list, delete, or retry jobs without authorization.
**Learning:** Admin endpoints were assumed to be protected by network boundaries or missed during authorization enforcement rollouts. Relying solely on network segmentation without application-level checks is a security gap.
**Prevention:** Always ensure every single controller endpoint explicitly requires a permission via `TenantAccessService.require(headers, TenantPermissions.XXX)`. Do not skip authorization checks for admin or internal-facing endpoints.
