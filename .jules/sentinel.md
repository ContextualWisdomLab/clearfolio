...
## 2026-07-28 - Missing Auth on Admin Controller Endpoints
**Vulnerability:** The `AdminController` endpoints (/api/v1/admin/convert/jobs, etc.) lacked `TenantAccessService` integration, meaning they required no authentication/authorization, allowing unauthorized users to retrieve all jobs (including other tenants') and delete/retry any conversion jobs.
**Learning:** Just because a controller handles administrative functionality doesn't mean it inherently inherits security controls. If a controller lacks explicit authorization checks, its endpoints are open to the world.
**Prevention:** Every endpoint handling sensitive operations or data must explicitly require appropriate permissions by injecting `TenantAccessService` and invoking `tenantAccessService.require(headers, TenantPermissions.XXX)` to prevent unauthorized access.
