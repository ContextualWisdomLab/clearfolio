...
## 2026-08-19 - Add Missing Authorization and Tenant Isolation to Admin Endpoints
**Vulnerability:** The `AdminController` endpoints (`/api/v1/admin/convert/jobs`, `deleteJob`, `retryDeadLettered`) did not verify caller permissions via `TenantAccessService`, allowing unauthenticated actors to access and manipulate all conversion jobs globally. Furthermore, jobs were not scoped to the caller's tenant, exposing cross-tenant metadata (data leak).
**Learning:** Even internal or admin-specific controllers must apply the same authentication, authorization, and tenant isolation layers as user-facing endpoints to adhere strictly to the principle of least privilege and prevent horizontal/vertical privilege escalation.
**Prevention:** Always verify endpoint permissions explicitly (e.g., using `TenantAccessService.require(headers, ...)`) and strictly enforce tenant scope (e.g., `job.belongsToTenant(tenantContext.tenantId())` or `requireSameTenant`) for every protected resource, including administrative actions.
## 2026-08-19 - Safe Endpoints Still Require Deep Service Boundaries
**Vulnerability:** Even when a controller enforces API-level authorization (via `TenantAccessService`), querying global inventory methods (`conversionService.getAllJobs()`) before filtering creates a data access boundary leak. If the collection is large or poorly filtered later, it still represents cross-tenant read. Additionally, performing targeted updates using globally addressable locators without passing down the authenticated security context allows backend logic to bypass strict domain isolation.
**Learning:** Security validation in the UI/controller layer is insufficient for robust defense. The backend services themselves must be parameterized with the authenticated security context (e.g. `TenantContext`) and enforce data-access filtering at the data source/repository level, ensuring deep, pervasive defense against authorization bypasses.
**Prevention:** Pass the authenticated caller context into internal service layers instead of manually filtering output at the controller. Ensure backend service methods natively enforce tenant scoping during data lookups and mutations.
