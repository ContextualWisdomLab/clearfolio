...
## 2026-09-10 - Add authentication to admin endpoint
**Vulnerability:** AdminController endpoints (`/api/v1/admin/convert/jobs`, etc.) were missing authentication and authorization checks, allowing unauthenticated users to access administrative functions like viewing all jobs, deleting jobs, or retrying jobs.
**Learning:** Spring `@RestController` endpoints are not automatically protected unless explicitly configured. Administrative endpoints must have strict authorization checks to prevent unauthorized access.
**Prevention:** Always inject `TenantAccessService` and explicitly call `require()` with the appropriate permission on every administrative endpoint.
