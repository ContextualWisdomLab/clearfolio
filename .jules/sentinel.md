...
## 2026-07-29 - Missing Authentication on Admin Endpoints
**Vulnerability:** The endpoints in `AdminController.java` (`/api/v1/admin/convert/jobs`, `DELETE /api/v1/admin/convert/jobs/{jobId}`, `POST /api/v1/admin/convert/jobs/{jobId}/retry`) were not enforcing any authentication or authorization checks.
**Learning:** Any endpoint, especially those performing sensitive administrative actions or accessing broad sets of data, must strictly enforce authentication and authorization. Relying on network segmentation or obscurity is insufficient.
**Prevention:** Always inject `TenantAccessService` and use `tenantAccessService.require(headers, TenantPermissions.[SPECIFIC_PERMISSION])` in all endpoints to ensure they are secured.
