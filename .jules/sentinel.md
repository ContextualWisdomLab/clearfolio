...
## 2026-09-20 - Length Extension and Canonicalization Vulnerability in Hash Payloads for Tenant Claims
**Vulnerability:** The HMAC-SHA256 signature payload for tenant claims in `TenantAccessService` was constructed by concatenating strings separated by newline characters: `context.tenantId() + "\n" + context.subjectId() + ...`. This allows attackers to craft ambiguous inputs if they embed a newline character `\n` inside their payload, potentially bypassing validation via canonicalization attacks.
**Learning:** Simple string concatenation, even with a newline delimiter, is insecure when generating cryptographic hashes or signatures for multiple inputs. Attackers can shift delimiters to produce identical payloads for entirely different logical inputs.
**Prevention:** Always use length-prefixing or unambiguous delimiters when combining multiple inputs for cryptographic hashing. For example, use `field.length() + ":" + field` to strictly define the boundaries of each field.

## 2026-09-20 - Missing Authentication on Administrative API Endpoints
