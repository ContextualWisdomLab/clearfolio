...
## 2026-09-25 - Missing Authorization Checks in AdminController
**Vulnerability:** The endpoints mapped in `AdminController.java` (`/api/v1/admin/convert/jobs`, etc.) did not use `TenantAccessService` to verify the request headers. Thus, any unauthenticated user or an authenticated user without admin permissions could list, delete, or retry conversion jobs across all tenants.
**Learning:** Spring controllers must explicitly inject `TenantAccessService` and invoke it to verify identity and require appropriate permissions because this project doesn't use standard Spring Security method annotations (like `@PreAuthorize`). Relying implicitly on the URL path for admin functions allows unauthorized direct object reference or access control bypasses.
**Prevention:** Always verify explicit tenant permissions on every controller endpoint using `tenantAccessService.require(headers, TenantPermissions.XXX)`. Define distinct permissions for administrative boundaries, e.g., `admin:access`.
