...
## 2025-02-21 - [CRITICAL] Fix missing authentication on admin endpoints
**Vulnerability:** The AdminController endpoints (`/api/v1/admin/convert/jobs`, `/api/v1/admin/convert/jobs/{jobId}`, `/api/v1/admin/convert/jobs/{jobId}/retry`) lacked any authentication and authorization checks, allowing unauthenticated attackers to read, delete, or retry conversion jobs across all tenants.
**Learning:** Admin endpoints were added without integrating the existing `TenantAccessService`, highlighting a gap in ensuring all new API surfaces default to being secure and enforcing access control.
**Prevention:** Ensure all new controllers and endpoints explicitly inject and utilize `TenantAccessService.require(...)` with an appropriate permission.
