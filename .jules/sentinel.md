...
## 2026-09-06 - Add missing authentication to Admin endpoints
**Vulnerability:** Admin endpoints for listing, deleting, and retrying conversion jobs lacked authentication and authorization checks, allowing any anonymous user to access and manipulate job state.
**Learning:** Relying purely on network security or implicit trust is insufficient. Every sensitive controller endpoint must explicitly verify authorization using the appropriate tenant service to ensure zero-trust security.
**Prevention:** Apply `tenantAccessService.require(headers, TenantPermissions.XXX)` consistently across all admin and standard controllers, ensuring that every request header is authenticated and authorized before executing operations.
