...
## 2026-08-29 - Missing Authorization on Administrative Endpoints
**Vulnerability:** The `/api/v1/admin/convert/jobs` endpoints were completely unauthenticated. Anyone could query all conversion jobs across all tenants, delete arbitrary jobs, or trigger retries. This is a critical Missing Authorization / IDOR vulnerability.
**Learning:** Internal or administrative endpoints are sometimes assumed to be protected by external network routing or API gateways, but relying solely on infrastructure without application-level checks violates Defense in Depth. The application itself must verify the identity and permissions of the caller.
**Prevention:** Always explicitly apply authentication and authorization checks (e.g., using `TenantAccessService`) on all administrative or internal endpoints, introducing dedicated administrative permissions (e.g., `admin:read`, `admin:write`) that are strictly enforced.
