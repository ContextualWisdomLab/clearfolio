...
## 2026-09-09 - Missing Authentication on Admin Endpoints
**Vulnerability:** The AdminController endpoints for reading, deleting, and retrying conversion jobs lacked any TenantAccessService authorization checks. This allowed unauthenticated actors to retrieve all tenant jobs or destructively retry/delete operations across the entire platform.
**Learning:** In a controller-level authorization design, missing explicit require() calls leaves sensitive administrative endpoints completely exposed. System components do not default to deny unless the access service is actively injected and invoked.
**Prevention:** Ensure that all endpoints—especially those serving admin or cross-tenant functions—explicitly inject TenantAccessService and invoke require() with proper TenantPermissions (e.g., ADMIN_READ, ADMIN_WRITE).
