...
## 2026-08-23 - 시크릿을 설정 트리로 마이그레이션
**Vulnerability:** application-buyer-demo.yml 파일과 자바 코드에 환경 변수를 통한 시크릿 의존성(`${...:}`)이 있어, 시크릿이 누락될 경우 빈 문자열로 초기화되는 보안 취약점 존재.
**Learning:** Spring Boot 설정 트리(config tree) 메커니즘을 사용해야 하며, @Value 애노테이션에서 기본값(fallback)을 빈 문자열로 제공하면 안 됨.
**Prevention:** 환경 변수 주입 방식 대신 config tree를 사용하고, @Value 애노테이션에 기본값 없이 설정하여 누락 시 fail-fast 하도록 강제.
