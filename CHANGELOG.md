...
- Maven `verify` 이후 Surefire 보고서가 존재하고 실행 테스트 수가 1개 이상이며 skipped 수가 0인지 검증합니다. Failsafe 보고서가 생성된 경우 동일한 규칙을 적용하며, 보고서 누락·손상·음수 카운트·전체 skip은 exact-head CI와 merge-compatibility 모두에서 fail closed 처리합니다.
...
- Maven XML 테스트 보고서 검증기는 파일당 16 MiB 상한을 적용하고 DTD·엔터티 선언을 파싱 전에 거부합니다. 테스트 코드가 보고서 경로를 조작해도 외부 엔터티 읽기와 엔터티 확장형 서비스 거부가 CI 검증 단계에 도달하지 못하도록 회귀 테스트로 고정했습니다.
