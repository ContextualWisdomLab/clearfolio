...
- **UI UX 개선**: 폼 필수 입력 표시, 빈 상태 안내문 추가 및 동적 컨테이너 접근성 개선
  - 파일 입력 라벨에 시각적 필수 표시(*)를 추가하고 접근성 속성을 부여했습니다.
  - 세션 히스토리의 빈 상태 문구에 행동 유도(Call-to-Action) 문구를 추가했습니다.
  - 동적으로 로드되는 `job-detail` 영역에 `tabindex="-1"`을 추가하여 포커스 관리가 가능하도록 개선했습니다.
