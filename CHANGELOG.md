...
- **UI UX 개선**: `aria-busy="true"`인 비동기 버튼과 PDF 미리보기 렌더링 대기 구간에 CSS 로딩 표시를 추가했습니다. 초기 스켈레톤이 표시되는 동안에는 미리보기 스피너를 중복 노출하지 않고, `prefers-reduced-motion: reduce` 환경에서는 회전 애니메이션을 중지합니다.
