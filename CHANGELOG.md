...
- **접근성 및 문서 입력 필드 필수 표시 개선**
  - 'Document' 입력 라벨 옆에 시각적인 필수 입력 표시(`*`)를 추가했습니다.
  - 이 표시로 인해 스크린 리더가 HTML5 `required` 속성과 중복으로 필수 여부를 읽는 문제를 방지하기 위해 `aria-hidden="true"`를 적용했습니다.
