...
## [0.1.1] - 2026-08-06
### 변경 사항
- **성능 개선**: `ArtifactLinkService.parseAndVerify` 내에서 서명 검증이 완료되기 전 불필요한 배열 및 객체 할당을 방지하기 위해 정규식(`String.split`) 대신 단일 스캔(`indexOf`)과 `substring`을 사용하여 성능을 최적화하고 가비지 컬렉션(GC) 부하를 줄였습니다.
