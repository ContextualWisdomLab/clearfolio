...
- `GET /api/v1/convert/jobs/{jobId}/download`가 리소스 조회 전에 `job:read` 권한을 검증하고, PDF 저장소 접근 전에 작업의 tenant 소유권을 확인하도록 강화했습니다. 인증 누락·권한 누락·교차 tenant UUID 접근은 각각 fail closed 처리되며, 교차 tenant 요청은 리소스 존재를 숨기는 `404`를 반환합니다.
