...
### Fixed
- 화면 판독기를 사용하는 사용자를 위해 반복적인 테이블 액션 버튼("세부 정보", "상태 JSON", "뷰어 열기")에 파일명이 포함된 동적인 `aria-label` 속성을 제공하여 접근성을 개선했습니다.
- "세부 정보" 버튼과 같은 비동기 액션 버튼이 "로딩 중..." 상태로 변경될 때 `aria-label`도 임시로 업데이트하여 스크린 리더 사용자가 로딩 상태를 인식할 수 있도록 하고, 작업 완료 후 원래 라벨로 올바르게 복원되도록 수정했습니다.
