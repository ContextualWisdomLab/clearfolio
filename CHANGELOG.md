...
- **시각적 상태 전환 효과(Transitions) 추가**: 뷰어 UI의 버튼(`.btn`), 폼 입력창(`.file-input`), 링크(`a`) 요소들의 배경색이나 색상 변경에 부드러운 전환 효과(transition)를 추가하여 시각적 피드백을 개선했습니다. 또한 키보드 내비게이션 시 포커스 링이 지연 없이 즉각적으로 나타나도록 보장하며, `prefers-reduced-motion: reduce` 설정이 활성화된 환경에서는 `.skip-link`의 이동(transform) 모션을 비활성화하여 접근성 지침을 준수하도록 구현했습니다.
