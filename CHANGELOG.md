...
- **시각적 상태 전환 효과(Transitions) 추가**: 뷰어 UI의 버튼(`.btn`), 폼 입력창(`.file-input`), 링크(`a`), 포커스(`:focus-visible`) 요소들에 상태 변경 시 부드럽게 시각적 변화가 일어나는 전환 효과(transition)를 추가하여 전반적인 인터랙션 경험을 개선했습니다.
