...
- 세션 히스토리가 비어 있을 때 `Clear history` 작업을 기본 비활성화하고, 보이는 이름과 접근 이름을 동일하게 유지하며, 모든 히스토리 렌더링 후 실제 항목 수와 버튼 가용성을 동기화합니다. 확인 후 삭제 시 키보드 초점은 `#history-title` 랜드마크로 돌아갑니다.
