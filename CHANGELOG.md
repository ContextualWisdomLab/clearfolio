...
- 관리자 작업 조회·삭제·재시도 API가 실제 `TenantAccessService`의 `admin:read`/`admin:write` 검증을 통과한 요청만 허용하고, 검증된 request tenant에 속한 작업만 조회·변경하도록 강화했습니다. 인증 누락은 `401`, 권한 누락은 `403`, 교차-tenant UUID 접근은 리소스 존재를 숨기는 `404`로 fail closed 처리하며, 재시도 감사 주체에는 고정 `admin` 문자열 대신 검증된 subject를 전달합니다.
