...
- Draft PR 게시용 GitHub App 설치 토큰이 설치 전체 권한을 상속하지 않도록 런타임에서 필요한 GitHub 권한 범주인 `contents: write`와 `pull-requests: write`만 명시적으로 요청합니다. GitHub가 브랜치·릴리스와 PR·병합 API를 각각 같은 범주에 묶으므로, 불변 패치·Draft 전용 명령·금지 명령 회귀 테스트·보호 규칙을 함께 적용해 실제 동작을 브랜치 게시와 Draft PR 생성으로 제한합니다.
- Draft PR 게시 러너의 네트워크 정책을 관찰 전용 `audit`에서 fail-closed `block`으로 변경하고 GitHub API·체크아웃·Actions 아티팩트 전송에 필요한 엔드포인트만 허용했습니다. GitHub App private key와 단기 쓰기 토큰을 취급하는 단계에서 임의 외부 호스트로 자격 증명을 반출할 수 있는 경로를 차단합니다.
- 자격 증명 단계의 편집 권한과 패키징 경계에서 `pom.xml`을 명시적으로 보호했습니다. 모델 출력이 의존성·Maven 플러그인·빌드 실행 입력을 바꾼 뒤 검증 러너에서 실행되는 공급망 경로를 차단합니다.
- 모델이 제안한 Java·Python 코드를 실행하는 자격 증명 없는 검증 러너의 네트워크 정책을 관찰 전용 `audit`에서 fail-closed `block`으로 변경했습니다. GitHub Actions·아티팩트 전송, Maven Central, 해시 고정 PyPI 설치에 필요한 엔드포인트만 허용해 제안된 테스트나 생산 코드가 임의 외부 호스트로 데이터를 전송하는 경로를 차단합니다.
