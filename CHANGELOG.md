...
- **UI 접근성 개선**: 동적으로 생성되는 공용 작업 버튼("Details", "Status JSON", "Open viewer")에 파일 이름을 포함하는 `aria-label` 속성을 추가하여 스크린 리더 사용자가 해당 버튼이 어떤 파일의 정보를 나타내는지 명확하게 식별할 수 있도록 컨텍스트를 제공했습니다.
