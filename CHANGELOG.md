...
- 아티팩트 토큰은 정확히 10개 payload 필드와 1개 서명을 요구하고 HMAC 확인 뒤 claim을 생성합니다. 서명은 유효하지만 필수 텍스트 claim이 비어 있거나 공백 문자만 포함한 payload는 `ArtifactTokenClaims` 생성 단계에서 거부되어 token-ledger 조회 전에 `401 Unauthorized`로 실패하며, claim 값을 정규화하지 않아 검증된 서명 의미를 보존합니다.
