...
| Nested-safe accessible asynchronous controls | historical #264 requires clean reconciliation | reusable `dom-utils.js` busy-state behavior exists only on predecessor ancestry | current-base JavaScript unit, integration, branch coverage, focus and restoration evidence required | PRD FR-09; UML viewer flows; issue #263 |
...
| Deterministic single logging runtime binding | `PLANNED`; issue #320 | protected main packages Spring `spring-jcl` plus standalone Commons Logging introduced through PDFBox | dependency tree, package classpath, warning-absence, PDFBox behavior and regenerated SBOM/attribution tests required | issue #320; OPERABILITY; RELEASE_ACCEPTANCE |
