...
| Is persistence production-grade? | Partial | `ConversionJobRepository` abstraction exists, and `docs/persistence/2026-07-02-durable-conversion-job-repository-plan.md` defines target tables, transition events, worker recovery, and migration sequence. | Durable SQL implementation is not built, and mutable worker state transitions are not yet persisted transactionally. | `ConversionJobStateStore` implementation and SQL repository profile. |
...
| Durable job repository design | `docs/persistence/2026-07-02-durable-conversion-job-repository-plan.md`; includes SQL target tables, lifecycle transition contract, worker recovery model, repository API change sequence, and buyer acceptance criteria. |
