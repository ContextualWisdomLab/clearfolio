...
| Is there license/SBOM evidence? | Partial | CycloneDX SBOM evidence exists under `docs/qa/evidence/2026-07-02-krw2b-sale-readiness/`; engineering review exists in `docs/security/2026-07-02-license-allowlist-review.md`; the unused broad Tika parser package was removed; `DependencyPolicyTest` prevents reintroduction; `scripts/check_sbom_license_policy.py` enforces the engineering allowlist and current policy summary reports 60 allowed components, 3 review-required components, and 0 unlisted violations. | Three flagged Spring runtime components still need legal approve, replace, or remove decisions before buyer-release mode can require zero review-required components. | Legal sign-off and buyer-release license-policy mode. |
...
1. Get legal sign-off or replacement decisions for the three remaining review-required SBOM
