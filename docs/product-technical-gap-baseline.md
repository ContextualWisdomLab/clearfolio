...
PR #568 contains a test-first descendant that adds cross-tenant list/delete/retry cases. Those tests require foreign tenant data to be absent from list responses, destructive operations to return `404` for missing or foreign aggregates, retry audit attribution to use the verified subject, and the post-ownership disappearance race to remain a `404`. The production descendant implements the minimum controller-side ownership enforcement and removes the one-shot `fix_javadoc.py` source-rewriting helper.

A later automation commit restored the pre-repair controller/tests/docs while attempting to retrigger a flaky check. The fleet repair adopted that intervening descendant rather than force-pushing and re-applied the valid tenant-isolation contract as ordinary descendants.
