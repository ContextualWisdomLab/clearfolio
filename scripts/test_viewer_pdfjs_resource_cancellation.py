...
async function assertRenderTaskIsCancelled(context) {
  ...
  assert.equal(
    cancelCalls,
    1,
    "supersession must actively cancel the current PDF.js RenderTask",
  );
  resolveRender();
  assert.equal(await rendering, false, "an aborted render must report failure");
  assert.equal(
    documentDestroyCalls,
    1,
    "completed render cleanup must still destroy the PDF document",
  );
}
...
async function assertLoadingTaskIsDestroyed(context) {
  ...
  assert.equal(
    loadingDestroyCalls,
    1,
    "supersession must destroy an in-flight PDFDocumentLoadingTask",
  );
}
...
class ViewerPdfJsResourceCancellationTest(unittest.TestCase):
    """Validate active cancellation through the supported Node.js runtime."""

    def test_pdfjs_tasks_are_actively_cancelled_on_supersession(self) -> None:
        """Require RenderTask cancellation and loading-task destruction."""
