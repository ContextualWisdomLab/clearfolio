...
function renderHistory(history = loadHistory()) {
  ...
  for (const job of history) {
    ...
    const fileDisplayName = job.fileName || "Document";
    if (job.statusUrl) {
      ...
      }, `Details for ${fileDisplayName}`));
      actionsCell.appendChild(createActionButton("Status JSON", () => {
        void openJsonDocument(job.statusUrl, "Clearfolio status JSON");
      }, `Status JSON for ${fileDisplayName}`));
    }
    if (job.jobId) {
      actionsCell.appendChild(createLink(`/viewer/${encodeURIComponent(job.jobId)}`, "Open viewer", `Open viewer for ${fileDisplayName}`));
    }
