...
function renderHistory(history = loadHistory()) {
  ...
  for (const job of history) {
    ...
    const documentName = job.fileName || "document";
    if (job.statusUrl) {
      ...
      }, `View details for ${documentName}`));
      actionsCell.appendChild(createActionButton("Status JSON", () => {
        void openJsonDocument(job.statusUrl, "Clearfolio status JSON");
      }, `Open status JSON for ${documentName}`));
    }
    if (job.jobId) {
      actionsCell.appendChild(createLink(`/viewer/${encodeURIComponent(job.jobId)}`, "Open viewer", `Open viewer for ${documentName}`));
    }
