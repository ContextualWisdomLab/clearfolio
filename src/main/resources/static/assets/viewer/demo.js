...
function renderHistory(history = loadHistory()) {
  ...
  for (const job of history) {
    ...
    const rowTitle = job.fileName || "Document";
    fileCell.textContent = rowTitle;
    ...
    if (job.statusUrl) {
      actionsCell.appendChild(createActionButton("Details", (e) => {
        ...
        openJobDetail(job).finally(() => {
          ...
          btn.setAttribute("aria-label", `Details for ${rowTitle}`);
          btn.disabled = false;
        });
      }, `Details for ${rowTitle}`));
      actionsCell.appendChild(createActionButton("Status JSON", () => {
        void openJsonDocument(job.statusUrl, "Clearfolio status JSON");
      }, `Status JSON for ${rowTitle}`));
    }
    if (job.jobId) {
      actionsCell.appendChild(createLink(`/viewer/${encodeURIComponent(job.jobId)}`, "Open viewer", `Open viewer for ${rowTitle}`));
    }
