...
function renderHistory(history = loadHistory()) {
  ...
  for (const job of history) {
    ...
    if (job.statusUrl) {
      ...
      }, `Details for job ${job.jobId}`));
      actionsCell.appendChild(createActionButton("Status JSON", () => {
        void openJsonDocument(job.statusUrl, "Clearfolio status JSON");
      }, `Status JSON for job ${job.jobId}`));
    }
    if (job.jobId) {
      actionsCell.appendChild(createLink(`/viewer/${encodeURIComponent(job.jobId)}`, "Open viewer", `Open viewer for job ${job.jobId}`));
    }
